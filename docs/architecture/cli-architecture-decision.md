...
2. 在 `docs/workflow/project-layout.md` 中把 `prototype_optimizer/` 标成 legacy/归档候选。
...
早期 `archive/tools-legacy-20260421/cst_cli.py` 直接加载 `mcp/advanced_mcp.py` 只是 POC 手段。它的问题是：
...
- 每个命令都有 `list-tools` / `describe-tool` / `args-template` / `invoke` 或等价发现机制。
- 输入是 JSON 或明确参数。
...
早期 `archive/tools-legacy-20260421/cst_cli.py` 直接加载 `mcp/advanced_mcp.py` 复用 `prepare_new_run`，这对 POC 可以接受，但不是长期架构。新的 runtime 迁移不继续往 `tools/` 增加入口，也不修改原有 `mcp/*.py` 源文件；先在 `cst_runtime/` 内另起边界。
...
| `project_identity.py` | `project_path` 规范化、run_dir 推断、打开工程列表、目标工程唯一性校验、锁文件等待 | `archive/tools-legacy-20260421/cst_cli.py` 中的 `_normalize_path`、`_infer_run_dir`、`_attach_expected_project` 思路 |
| `cli.py` / `__main__.py` | runtime 自带 CLI adapter，提供 `list-tools`、`describe-tool`、`args-template`、`invoke` 与直接命令入口 | 新建，不放入 `tools/`，避免辅助脚本目录继续膨胀 |
| `modeler.py` | 优化闭环所需的 modeler 运行能力：打开/关闭/保存工程、参数读写、仿真启动与轮询 | 只迁移优化流程运行能力，不迁移几何建模、材料、边界、网格等建模工具 |
| `results.py` | 优化闭环所需的 results 读取能力：打开结果工程、run_id、参数组合、1D/2D JSON 导出、S11 对比 HTML、farfield TXT 预览解析 | 直接使用 `cst.results` 和结构化导出文件，不依赖旧脚本 |
| `farfield.py` | runtime 远场试迁移：fresh-session 导出、Realized Gain dBi 网格读取、farfield ASCII 检查、近轴 flatness 计算 | 迁移 results-MCP 中已验证思路，但生产替代仍需实机验收 |
...
- `cli.py` / `__main__.py`：已支持 `python -m cst_runtime list-tools`、`describe-tool`、`args-template`、`invoke` 与直接工具命令。
...
- 非法 JSON 入参返回结构化 `invalid_json_args`，不再污染 stdout 为 traceback；复杂 PowerShell JSON 不再推荐手写，优先用 `args-template --tool ... --output ...` 生成 args 文件骨架。
...
- 不把已归档的 `archive/tools-legacy-20260421/cst_cli.py` 扩展成第二套完整生产链。
...
- 不在远场 fresh-session 实机验收和多任务低上下文验证完成前，把 runtime 宣称为完整生产替代。
