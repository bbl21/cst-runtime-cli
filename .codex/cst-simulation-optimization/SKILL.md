...
3. 在当前 GUI 会话中调用可见 MCP tool call：`cst_results_http.export_farfield_fresh_session(...)`。必须显式传入：
   ...
   - `plot_mode`: 通常传 `Gain` 或 `Efield`；工具内部按 E-field components 导出完整网格
...
- `export_farfield_fresh_session` 的内部已验证链路：`fresh CST process + fresh open project + FarfieldPlot.AddListEvaluationPoint + CalculateList + GetListItem + close(save=False)`。
