...
- 本 Skill 是当前 MCP 稳定生产链的执行流程、MCP 调用链、失败恢复和任务闭环主入口
- 若用户明确要求 CLI/runtime 或低上下文 CLI 验证，应改用 `skills/cst-runtime-cli-optimization/SKILL.md`
...
## 与 CLI runtime Skill 的并行关系

- 当前默认生产链仍保留 MCP，尤其是远场、真实 gain/dBi、GUI 可见验收和历史任务回归。
- `cst-runtime-cli-optimization` 是并行迁移链，不替代本 Skill，除非用户明确选择 CLI/runtime。
- 两条链必须共享 `tasks/task_xxx_slug/runs/run_xxx/` 结构、`status.json`、`logs/`、`stages/` 和 `exports/`。
- CLI 全量能力完成并通过低上下文验证前，不推动 MCP 退场。

## 工具粒度与 agent 判断

- 固定、长流程、机械重复且输入输出稳定的部分，可以调用阶段级封装工具，减少重复机械调用。
- 需要根据返回信息做判断的步骤，必须保留小粒度工具接口，由 agent 按本 Skill 判断下一步。
- 阶段级封装必须返回结构化事实，而不是只返回 `success`：至少包含关键输入、输出文件、状态、错误分类、指标摘要和审计日志路径。
- agent 拿到工具返回后，应判断是否继续、重试、换参数、切换策略、暂停或询问用户；不要把优化流程当成无判断的固定脚本。
- 小粒度工具和阶段级封装都必须写入当前 run 的正式产物目录，禁止把返回只留在对话或 stdout 中。
...
- `summary.md` 记录结论摘要
- `status.json` 记录当前状态
- 主链阶段记录统一通过 `cst-modeler.record_run_stage(...)` 写入 `stages/` 与 `logs/production_chain.md`
- `status.json` 的生产状态更新统一通过 `cst-modeler.update_run_status(...)` 完成，禁止把手写 JSON 当作默认生产链路
...
1. 确认 `cst-results-http` 已启动，或确认自动化编排器已能按需拉起 results MCP runtime。
2. 确认工具列表包含 `export_farfield_fresh_session`；交互式调试、开发验收或用户明确要求可见时，优先用当前 GUI 会话中的可见 MCP tool call 证明服务可用。
3. 调用正式 MCP 工具：`cst_results_http.export_farfield_fresh_session(...)`。自动化生产流程允许由编排器内部调用，不要求向用户暴露每一步 tool call；但必须把调用摘要、关键参数、输出路径、状态、错误和必要的 `flow_log` 写入当前 run 的 `logs/`、`stages/` 或 `status.json`。必须显式传入：
...
- `tools/call_local_mcp_tool.py` 和直接函数调用只允许开发期定位问题；远场导出生产过程必须走正式 results MCP 工具链。
- 交互式调试、开发验收或用户明确要求可见时，优先使用当前 GUI 可见 MCP tool call；自动化生产允许内部调用 MCP，但必须保留 run 内审计记录，不能变成无日志的黑盒执行。
- `tools/call_mcp_tool.py` 只作为命令行 MCP client 排障和自动化验收辅助，不能替代正式主链的状态落盘与结果验证。
...
1. `cst-modeler.prepare_new_run(task_path=...)` 创建标准 run、复制工程副本并初始化 `config.json` / `status.json` / `summary.md`。
2. `cst-modeler.get_run_context(task_path=..., run_id=...)` 读取当前 run 的 `working_project`、`exports_dir`、`logs_dir`、`analysis_dir`，后续路径一律来自这里。
3. modeler 侧只负责打开工作副本、修改参数、启动仿真、关闭项目和 CST 进程。
4. results 侧只负责刷新 session、读取结果、导出 JSON/TXT、生成 HTML 对比或预览。
5. 每个关键阶段完成后调用 `cst-modeler.record_run_stage(...)`；最终或阶段性状态调用 `cst-modeler.update_run_status(...)`。
6. `tmp/` 和 `archive/tools-legacy-20260421/` 下的旧旁路/调试脚本只能作为历史参考，不能作为默认生产节点。

### Runtime CLI 迁移状态（2026-04-21）

当前已新增 `cst_runtime/` 作为 MCP 之外的共享运行层，入口为：
...
- run 与审计：`prepare-run`、`get-run-context`、`record-stage`、`update-status`
- 项目身份：`verify-project-identity`、`wait-project-unlocked`
- modeler 运行能力：`open-project`、`list-parameters`、`change-parameter`、`is-simulation-running`、`close-project`
- results 读取能力：`open-results-project`、`list-run-ids`、`get-parameter-combination`、`get-1d-result`、`generate-s11-comparison`

边界：

- 不迁移几何建模、材料、边界、网格等建模工具。
- 不修改原有 `mcp/*.py` 源文件来完成第一阶段迁移。
- `start-simulation-async -> is-simulation-running -> save-project -> close-project -> wait-project-unlocked -> list-run-ids -> get-1d-result` 已在临时 run 中完成真实仿真实测。
- 该 runtime 链路可作为优化闭环的 CLI-first 迁移基础，但在远场导出和低上下文迁移包验证完成前，不得宣称完整替代全部生产链。
- 远场导出仍使用既有 results-MCP 正式链路，不在当前 runtime 迁移范围内。
...
5. 调用 `cst-modeler.get_run_context(...)` 获取后续所有正式输出目录
6. 若工具返回锁文件或复制失败，先关闭相关 CST 窗口，再重新执行
...
1. 当前轮结果、参数组合、指标文件路径写入当前 run 的 `logs/` 或 `analysis/`
2. 调用 `cst-modeler.record_run_stage(...)` 记录阶段完成情况
3. 调用 `cst-modeler.update_run_status(...)` 合并 `best_result`、`output_files`、`status`、`stage`
...
3. 更新 `summary.md`
4. 调用 `cst-modeler.update_run_status(...)` 更新 `status.json`（含 best_result）
5. 把卡点写入 `logs/`
...
1. 对每个需要对比的 run_id，调用：
   ```
   cst-results.get_1d_result(
       treepath="1D Results\\S-Parameters\\S1,1",
       run_id=N,
       export_path="当前run/exports/s11_runN.json"
   )
   ```

2. 调用正式 MCP 工具：
   ```
   cst-results.generate_s11_comparison(
       file_paths=["当前run/exports/s11_run1.json", "..."],
       output_html="当前run/exports/s11_comparison.html"
   )
   ```

3. 调用 `cst-modeler.update_run_status(...)` 把 `s11_comparison_html` 登记到 `output_files`。

旧脚本 `archive/tools-legacy-20260421/generate_s11_comparison.py` 只保留为历史参考，不作为生产流程默认入口。
...
每次优化完成后，应生成交互式 S11 对比网页。生产流程只走 MCP：

1. `cst-results.get_1d_result(..., export_path="当前run/exports/*.json")`
2. `cst-results.generate_s11_comparison(file_paths=[...], output_html="当前run/exports/*.html")`
3. `cst-modeler.update_run_status(..., output_files_json=...)`

历史脚本 `archive/tools-legacy-20260421/generate_s11_comparison.py` 不再承担生产职责。
...
### 脚本定位调整
- `archive/tools-legacy-20260421/generate_s11_comparison.py` 仅用于历史参考。
