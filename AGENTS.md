...
- 生产过程必须优先使用当前 GUI 会话中可见的 MCP tool call（例如 `cst_results_http.export_farfield_fresh_session`），让用户能看到实际工具调用和返回；`tools/call_mcp_tool.py` 仅作为命令行 MCP client 排障备用，不能替代生产过程展示和最终交付依据
