...
> 它不是规则源；规则与红线仍以 `AGENTS.md` 为准，执行流程以当前维护的执行 Skill 和对应 MCP/runtime 文档为准。
...
| Skill | 正式流程、调用顺序、失败恢复、执行约束 | 不沉淀 Python 复用代码，不替代 MCP 工具 | 当前维护的执行 Skill；当前为 `skills/cst-runtime-cli/SKILL.md` 和 agent 生效副本 |
| 知识系统 | 规则、经验、长期共识、专题说明的分层入口 | 不直接覆盖 `AGENTS.md` 规则，不承担当次任务日志 | `AGENTS.md`、`MEMORY.md`、`docs/`、`docs/topic-index.md` |
| 计划系统 | 当前主线、阶段目标、门控条件和清单维护 | 不替代执行流程，不把长期愿景直接推入近期主线 | `docs/project-goals-and-plan.md`、`advanced/planning/current-priority-checklist.md` |
