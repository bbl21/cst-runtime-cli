...
6. `tmp/`、`tools/generate_s11_comparison.py`、`tools/e2e_*.py`、`tools/diag_refresh.py`、`tools/explore_results.py`、`tools/plot_farfield.py` 只能作为调试/历史辅助，不能作为默认生产节点。
...
`tools/generate_s11_comparison.py` 只保留为历史调试脚本，不作为生产流程默认入口。
...
历史脚本 `tools/generate_s11_comparison.py` 不再承担生产职责。
