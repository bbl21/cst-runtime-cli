...
```powershell
# 1. 全量自检 + 自动修复（初始化 workspace、配置 CST 库路径）
...
**auto-fix + uv sync 完成后，所有后续命令必须使用 `uv run` 模式**（`uv run python -m cst_runtime ...`），不可再走系统 Python，否则可能出现库冲突等不可预期结果。
...
auto-fix + uv sync 完成后所有后续命令使用 uv run python -m cst_runtime ...，不可再走系统 Python。
