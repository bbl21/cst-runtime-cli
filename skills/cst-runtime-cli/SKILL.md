...
从市场安装 Skill 后，coding agent 应自动执行以下流程：
...
`health-check` 会自动诊断并修复：
| 检查项 | 自动修复 | 无法修复时提示用户 |
|---|---|---|
| Python ≥ 3.13 | ❌ | 安装 Python 3.13+ |
| uv 包管理器 | ❌ | 安装 uv |
| 工作区初始化 | ✅ `init-workspace` | — |
| CST Python 库 | ✅ 扫描路径 + 配置 pyproject.toml | 安装 CST Studio Suite |
| 导入验证 | ✅ 自动重试 | 检查 CST 安装 |
...
检查项：python 版本、uv、工作区初始化、CST 安装扫描、pyproject.toml 配置、CST 导入验证。
...
```powershell
# 扫描可用安装（不修改）
python <skill-root>\scripts\cst_runtime_cli.py install-cst-libraries --dry-run true

# 自动检测并配置
python <skill-root>\scripts\cst_runtime_cli.py install-cst-libraries
